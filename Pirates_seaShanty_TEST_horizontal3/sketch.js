...
function draw() {
  ...
  push();
    ...
    push();
      ...
      //little arrow ends
      line(0, height/3*2, 30, (height/3*2)-30);
      line(0, height/3*2, 30, (height/3*2)+30);

      line(width, height/3*2, width-30, (height/3*2)-30);
      line(width, height/3*2, width-30, (height/3*2)+30);

      line(width/2, height/3, width/2-30, height/3+30);
      line(width/2, height/3, width/2+30, height/3+30);

      line(width/2, height, width/2-30, height-30);
      line(width/2, height, width/2+30, height-30);
    pop();
    //textALIGN(LEFT, TOP);
    text('trash', 0, (height/3*2)-10);
    text('treasure', width-60, (height/3*2)-10);
    text('web', width/2-10, height/3);
    text('sea', width/2-10, height-20);
  ...
  for (let i=0; i<callGenerated.length; i++) {
      ...
      for (let j=0; j<responseGenerated.length; j++) {
          ...
          //all call[i] text boxes added + final 200
          textXLength = ((40+(310*i)+300));
      }
...
function mousePressed() {
  ...
  if (odd==false) {
    ...
    if (textXLength>=width) {
        newWidth = textXLength;
        resizeCanvas(newWidth, height);
